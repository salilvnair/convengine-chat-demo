...
function buildGeneratedCode(settings, iconSvgs) {
  ...
  const lines = [
    ...
    // Audit switches — each prints only when it differs from the library
    // default, like the rest of this snippet.
    settings.showAuditSearch === false ? `    showAuditSearch: false,` : null,
    (settings.showAuditExplorer ?? true) ? `    showAuditExplorer: true,` : null,
    (settings.showAuditExplorer ?? true) ? `    auditExplorerUrl: '/audit',   // opened as /audit?conversationId=<current>` : null,
    settings.fullscreenTabAudit ? `    fullscreenTabUrl: '/fullscreen?audit=true',` : null,
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <Toggle checked={settings.showAuditSearch ?? true}  onChange={(v) => onChange({ ...settings, showAuditSearch: v })}  label="Audit Search Box"       hint="config.showAuditSearch"  modes={['fullscreen']}  accentColor={settings.accentColor} onLabelClick={() => scrollToConfigProp('showAuditSearch')} />
          <Toggle checked={settings.fullscreenTabAudit ?? false} onChange={(v) => onChange({ ...settings, fullscreenTabAudit: v })} label="Fullscreen Tab Is Auditable" hint="adds ?audit=true" modes={['panel','sidepanel']} accentColor={settings.accentColor} onLabelClick={() => scrollToConfigProp('fullscreenTabUrl')} />
          <Toggle checked={settings.showAuditExplorer ?? true} onChange={(v) => onChange({ ...settings, showAuditExplorer: v })} label="Audit Explorer Button" hint="config.showAuditExplorer" modes={['panel','sidepanel','fullscreen']} accentColor={settings.accentColor} onLabelClick={() => scrollToConfigProp('showAuditExplorer')} />
            ...
            <button
              onClick={() => {
                ...
                const p = new URLSearchParams({
                  ...
                  auditSearch:      String(settings.showAuditSearch ?? true),
                  auditExplorer:    String(settings.showAuditExplorer ?? true),
